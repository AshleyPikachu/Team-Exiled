...
class Player {
	constructor(user, starter) {
		...
		this.pc = [
			...
			[],
			[],
			["HoeenHero|ludicolo|||scald,gigadrain,icebeam,raindance|Jolly||M|20,30,23,3,30,28||50|0"],
			[],
			[],
			[]
		];
...
exports.commands = {
	...
	playalpha: function (target, room, user, connection, cmd) {
		...
		if (cmd === 'playalpha') {
			...
			try {
				...
				let newObj = new Player(user.userid, Tools.fastUnpackTeam(SG.makeWildPokemon(false, {
					name: "ERROR!",
					species: "Mudkip",
					level: 10,
					ability: 0
	...
	sggame: {
		...
		pc: function (target, room, user, connection, cmd) {
			...
			if (slot && !isNaN(Number(slot)) && Number(slot) > -1 && Number(slot) < 30 && Db('players').get(user.userid).pc[Number(box) - 1][Number(slot)] && !target[2] && target[0].split('|')[0] !== 'party') {
				orders = {
					withdraw: (Db('players').get(user.userid).party.length < 6),
					release: true,
					back: true
				};
			}
			...
			case 'withdraw':
				if (target[0].split('|')[0] === 'party') {
					target[2] = '';
					orders = {
						deposit: (Db('players').get(user.userid).party.length > 1),
						release: (Db('players').get(user.userid).party.length > 1),
						back: true
					};
				}
				break;
	...
	pickstarter: function (target, room, user) {
		...
		let obj = new Player(user, Tools.fastUnpackTeam(SG.makeWildPokemon(false, {
			species: target,
			level: 10,
			ability: 0
		})));
