...
const shop = [
	...
	['XY-Generations', 'Get three cards from the Generations pack released in the Pokemon XY set.', 4]
];
...
exports.commands = {
	...
	tradecard: function (target, room, user) {
		...
		if (Users.get(targetUser)) Users.get(targetUser).send("|pm|~GlaciaCardTradeClient|" + targetUser + "|/html <div class=\"broadcast-green\">" + Chat.escapeHTML(user.name) + " has initiated a trade with you.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
		user.send("|pm|~GlaciaCardTradeClient|" + user.userid + "|/html <div class=\"broadcast-green\">Your trade with " + Chat.escapeHTML(targetUser) + " has been initiated.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
	},
