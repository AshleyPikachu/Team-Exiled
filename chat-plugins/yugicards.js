...
exports.commands = {
	...
	buybooster: function (target, room, user) {
		...
		if (room.id !== 'lobby' && room.id !== 'dicehouse') room.addRaw(user.name + ' has bought <b>' + target + ' pack </b> from the shop.');
		room.update();
	},
	...
	tradeduelcard: function (target, room, user) {
		...
		if (Users.get(targetUser)) Users.get(targetUser).send("|pm|~CardTradeClient|" + targetUser + "|/html <div class=\"broadcast-green\">" + Chat.escapeHTML(user.name) + " has initiated a trade with you.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
		user.send("|pm|~CardTradeClient|" + user.userid + "|/html <div class=\"broadcast-green\">Your trade with " + Chat.escapeHTML(targetUser) + " has been initiated.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
	},
	...
	spawndualcard: function (target, room, user, connection, cmd) {
		...
		this.room.modlog(user.name + "gave the duelcard '" + dualcard.name + "' to " + targetUser + ".");
	},
	...
	takeduelcard: function (target, room, user, connection, cmd) {
		...
		this.room.modlog(user.name + " took the duelcard '" + dualcard.name + "' from " + targetUser + ".");
	},
};
