...
/********************
 * Cards
 * Avalon's Card System: Credit to Naten, nineage, fender, and everyone who added cards/
...
exports.commands = {
	...
	tradecard: function (target, room, user) {
		...
		if (Users.get(targetUser)) Users.get(targetUser).send("|pm|~AvalonCardTradeClient|" + targetUser + "|/html <div class=\"broadcast-green\">" + Chat.escapeHTML(user.name) + " has initiated a trade with you.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
		user.send("|pm|~AvalonCardTradeClient|" + user.userid + "|/html <div class=\"broadcast-green\">Your trade with " + Chat.escapeHTML(targetUser) + " has been initiated.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
	},
	...
	cardshelp: function (target, room, user) {
		if (!this.runBroadcast()) return;
		return this.sendReplyBox("<center><b><u>Avalon Trading Card Game:</u></b></center><br>" +
